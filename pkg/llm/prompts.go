...
- CRITICAL WEAK QUANTIFICATIONS: Do NOT generalize single data points into patterns. If the data shows "built team from 0 to 5 engineers" at ONE company, do NOT claim "built and scaled platform engineering teams" (plural) as if it's a pattern. Either use the specific context ("Founded Scribd's Security Team, scaling from 0 to 5 engineers") OR drop the numbers entirely and say "Built and scaled platform engineering teams" without quantification. Weak numbers (under 10) that aren't impressive should be omitted. If you can't make a strong quantitative claim, make a qualitative one instead.
- CRITICAL YEARS OF EXPERIENCE: use the EXACT number from profile.years_experience field. Do NOT round up, inflate, or estimate. If profile.years_experience is 25, you must say "25+ years" NOT "30+ years" or "over 25 years". This is factual accuracy - getting the number wrong is lying on the resume.
